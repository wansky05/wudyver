...
class RemakerAI {
  ...
  async img2img({ imageUrl, prompt, ...rest }) {
    try {
      ...
      const response = await axios.post(
        'https://api.remaker.ai/api/pai/v4/ai-toanime/appapi/create-job',
        form,
        { headers }
      );
      ...
      return {
        success: response.data.code === 100000,
  ...
  async img2img_status({ task_id }) {
    try {
      console.log(`Checking img2img status for job: ${task_id}`);
      
      const headers = { ...this.defaultHeaders };
      const response = await axios.get(
        `https://api.remaker.ai/api/pai/v3/ai-toanime/appapi/get-job/${task_id}`,
        { headers }
      );
  ...
  async txt2img({ prompt, model = 'anime', ratio = '9:16', ...rest }) {
    try {
  ...
  async txt2img_status({ task_id }) {
    try {
      ...
      const headers = { ...this.defaultHeaders };
      const response = await axios.get(
        `https://api.remaker.ai/api/pai/v4/ai-anime/get-job/${task_id}`,
        { headers }
      );
