...
class SpeechSynthesizer {
  ...
  async generate({ text, voice = 'alloy', ins = '', format = 'mp3', ...rest }) {
    const payload = {
      input: text,
      voice,
      instructions: ins,
...
export default async function handler(req, res) {
  ...
  try {
    const ttsGenerator = new SpeechSynthesizer(); // Mengubah nama kelas
    
    // Metode yang dipanggil sekarang adalah `generate`
