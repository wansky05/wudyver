    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      try {
        ...
        if (!targetProvince) {
          ...
          return {
            status: false,
            message: `Province '${prov}' not found. Available: ${provinces.map(p => p.name).join(", ")}`
          };
        }
        ...
        if (!targetRegency) {
          ...
          return {
            status: false,
            message: `Regency '${kab}' not found in ${targetProvince.name}. Available: ${normalizedRegencies.map(r => r.name).join(", ")}`
          };
        }
        ...
        if (!targetDistrict) {
          ...
          return {
            status: false,
            message: `District '${kec}' not found in ${targetRegency.name}. Available: ${districts.map(d => d.name).join(", ")}`
          };
        }
        ...
        if (!targetVillage) {
          ...
          return {
            status: false,
            message: `Village '${desa}' not found in ${targetDistrict.name}. Available: ${villages.map(v => v.name).join(", ")}`
          };
        }
        ...
        if (convertedFullResult.status === false) {
          ...
          return {
            status: false,
            message: convertedFullResult.message || "An error occurred during search (API responded with false status).",
        ...
        if (attempt === MAX_ATTEMPTS) {
          return {
            status: false,
            message: `Search failed after ${MAX_ATTEMPTS} attempts due to an unexpected error: ${error.message}`,
            data: [],
            matched: null
          };
        }
