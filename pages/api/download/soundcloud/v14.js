...
class SoundCloudMP3 {
  ...
  async getToken() {
    try {
      ...
      const { data } = await this.client.get('/converter');
  ...
  async download({ url, maxRetries = 5, delay = 2000 }) {
    try {
      ...
      const { data } = await this.client.post('/converter', params.toString(), {
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          const { data: statusData } = await this.client.get('/converter');
  ...
  parseResult($) {
    try {
      ...
      const info = $('.info.clearfix').eq(1); // Get the second info div
      
      // Extract all p elements and process them
