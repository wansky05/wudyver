...
const HIAI_AI_API_BASE = "/api/ai/gohiai";
...
const HiaiChatPage = () => {
  ...
  const callHiaiApi = async (url, method = "GET", bodyPayload = null) => {
    ...
    try {
      ...
      if (isSearchActionCall && responseData.characters && Array.isArray(responseData.characters)) {
        return { success: true, data: responseData.characters };
      } else if (isChatActionCall && responseData.text !== undefined) {
      ...
      console.error("Hiai AI API call error:", err);
  ...
  const handleSearchCharacter = async (e) => {
    ...
    const apiResponse = await callHiaiApi(`${HIAI_AI_API_BASE}?action=search&query=${encodeURIComponent(searchTerm)}`);

    // Clear search term after initiating search
    setSearchTerm("");
    ...
    if (apiResponse.success && Array.isArray(apiResponse.data)) {
      const formattedResults = apiResponse.data.map(item => ({
        id: item.characterId,
        name: item.nick,
        description: item.intro || 'Tidak ada deskripsi.',
        characterImageUrl: item.avatar && item.avatar.url ? item.avatar.url : "/assets/images/users/user-0.jpg",
        chatSessionCount: item.stat?.participation || 0, // Assuming participation maps to chat sessions
        createdBy: item.author?.nick || 'Anonim', // Using author nick for 'createdBy'
      }));
  ...
  const handleSendMessage = async (e) => {
    ...
    const apiResponse = await callHiaiApi(
      `${HIAI_AI_API_BASE}?action=chat&char_id=${selectedCharacter.id}&prompt=${encodeURIComponent(currentPrompt)}`,
      "GET"
    );

    if (apiResponse.success && typeof apiResponse.data === 'string') {
      setChatMessages((prevMessages) => [
        ...prevMessages, {
          id: Date.now() + 1,
          sender: "ai",
      ...
      >
          <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-700/60">
            ...
            <p className="text-sm text-center sm:text-left text-slate-500 dark:text-slate-400 mt-2 ml-0 sm:ml-[calc(2.5rem+1rem)] md:ml-[calc(3rem+1rem)]">
              Jelajahi dunia AI dan berbincang dengan karakter Hiai!
            </p>
          </div>
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <span className="text-xs text-slate-400 dark:text-slate-500 block truncate">
                                Pesan: {character.chatSessionCount} | Oleh: {character.createdBy}
                              </span>
                            </div>
                          </li>
                        ))}
