...
class EncoderClient {
  ...
  formatAsUUID(str, length = 8) {
    const hash = CryptoJS.SHA256(str);
    const hexHash = hash.toString(CryptoJS.enc.Base64url);
    const alphaOnly = hexHash.replace(/[0-9]/g, "");
    const taskID = alphaOnly.substring(0, length);
    uuidMapping.set(taskID, str);
    return taskID;
  }
  parseFromUUID(uuid) {
  ...
  clearCache() {
    uuidMapping.clear();
    console.log("Cache berhasil dikosongkan.");
  }
  getCacheSize() {
  ...
  logCache() {
    ...
    console.log("Ukuran cache saat ini:", this.getCacheSize());
    for (const [key, value] of uuidMapping.entries()) {
      console.log(`UUID: ${key}`);
      console.log(`Data: ${value}`);
      console.log("------------------");
    }
    console.log("--- Akhir Cache UUID ---");
  }
}
