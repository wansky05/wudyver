...
class EncoderClient {
  ...
  formatAsUUID(str) {
    ...
    this.uuidMapping.set(uuid, str);
    return uuid;
  }
  parseFromUUID(uuid) {
    if (!this.uuidMapping.has(uuid)) {
  ...
  getCacheSize() {
    return this.uuidMapping.size;
  }
  exportCache() {
    return JSON.stringify(Array.from(this.uuidMapping.entries()));
  }
  importCache(jsonData) {
    const data = JSON.parse(jsonData);
    this.uuidMapping = new Map(data);
  }
}
